...
Gdy ten warunek jest spełniony, granica istnieje. Dodatkowo, jeśli wartość własna $\lambda=1$ jest \textbf{prosta} (ma krotność algebraiczną równą 1), to łańcuch jest ergodyczny, a macierz graniczna ma postać, w której wszystkie wiersze są identyczne. 



\subsection{Struktura macierzy granicznej}
...
\begin{proof}
    Niech $n_0$ będzie takie, że $P^{n_0} > 0$. Zastosujmy twierdzenie Perrona do macierzy $P^{n_0}$. Wynika z niego, że $\rho(P^{n_0})=1$ jest prostą i dominującą wartością własną macierzy $P^{n_0}$. Musimy pokazać, że te własności przenoszą się na macierz $P$.
    ...
    \begin{obserwacja}
        ...
        \begin{proof}
        $A\vec{x} = \lambda\vec{x} \implies A^2\vec{x} = A(A\vec{x}) = A(\lambda\vec{x}) = \lambda(A\vec{x}) = \lambda^2\vec{x}$. Dowód przez indukcję jest trywialny.
        \end{proof}
    \end{obserwacja}

    \textbf{1. Dominacja:} Dowód przez sprzeczność. Załóżmy, że $\lambda=1$ nie jest dominująca dla $P$. Oznacza to, że istnieje inna wartość własna $\tilde{\lambda} \in \sigma(P)$ taka, że $|\tilde{\lambda}|=1$ i $\tilde{\lambda} \neq 1$.
    Zgodnie z obserwacją, $\tilde{\lambda}^{n_0}$ jest wartością własną macierzy $P^{n_0}$. Co więcej, $|\tilde{\lambda}^{n_0}| = |\tilde{\lambda}|^{n_0} = 1^{n_0} = 1$.
    Otrzymaliśmy dwie różne wartości własne $P^{n_0}$ o module 1: jedną równą $1^{n_0}=1$, a drugą $\tilde{\lambda}^{n_0}$. To stoi w sprzeczności z twierdzeniem Perrona dla macierzy $P^{n_0}>0$, które gwarantuje, że $1$ jest jedyną wartością własną o module 1. Zatem nasze założenie musiało być fałszywe.

    \textbf{2. Prostota:} Dowód przez sprzeczność. Załóżmy, że $\lambda=1$ nie jest prostą wartością własną $P$. Oznacza to, że jej krotność geometryczna jest większa od 1, czyli istnieją co najmniej dwa liniowo niezależne wektory własne $\vec{v}_1, \vec{v}_2$ dla $\lambda=1$. Ale te wektory są również wektorami własnymi macierzy $P^{n_0}$ dla wartości własnej $1^{n_0}=1$. To implikuje, że krotność geometryczna wartości własnej 1 dla macierzy $P^{n_0}$ jest co najmniej 2. To ponownie stoi w sprzeczności z twierdzeniem Perrona, które mówi, że dla $P^{n_0}>0$ wartość własna 1 jest prosta.
...
\subsection{Przypadki patologiczne: przywiedlność i okresowość}

Warunek regularności ($P^n > 0$ dla pewnego $n$) jest kluczowy dla prostych wyników zbieżności. Może on jednak nie być spełniony z dwóch powodów:

\textbf{1. Łańcuch jest przywiedlny (redukowalny):} Jeśli graf stanów nie jest silnie spójny, oznacza to, że istnieją rozłączne klasy komunikujące się. Gdy proces znajdzie się w jednej z takich klas, nigdy jej nie opuści. W konsekwencji, macierz $P^n$ nigdy nie będzie miała wszystkich elementów ściśle dodatnich, ponieważ przejścia między tymi klasami są niemożliwe.

\textbf{2. Łańcuch jest okresowy (mimo że nieprzywiedlny):} Łańcuch może być nieprzywiedlny (każdy stan jest osiągalny z każdego innego), ale wciąż nieregularny. Dzieje się tak, gdy powroty do stanów mogą następować tylko w wielokrotnościach pewnej liczby $d > 1$. Wtedy macierz $P^n$ nigdy nie stanie się ściśle dodatnia, ponieważ zera będą pojawiać się w sposób cykliczny, co doskonale ilustrują macierze permutacji.

\begin{example}
    Macierze permutacji generują okresowe łańcuchy.
    $P_1 = \begin{pmatrix} 0 & 1 \\ 1 & 0 \end{pmatrix}$, $P_2 = \begin{pmatrix} 0 & 1 & 0 \\ 0 & 0 & 1 \\ 1 & 0 & 0 \end{pmatrix}$.
    Potęgi tych macierzy nigdy nie staną się dodatnie, ponieważ zera pozostaną na stałych pozycjach, jedynie przesuwając się cyklicznie. To prowadzi do pojęcia okresu.
\end{example}

\begin{definition}[Okres stanu]
    Okresem stanu $j \in S$ nazywamy liczbę:
    $$ o(j) = \NWD \{ n \ge 1 : P_{jj}^{(n)} > 0 \} $$
    czyli największy wspólny dzielnik długości wszystkich ścieżek prowadzących ze stanu $j$ do samego siebie. Jeśli zbiór $\{...\}$ jest pusty, przyjmujemy $o(j)=\infty$.
    \begin{itemize}
        \item Jeśli $o(j) = 1$, stan $j$ nazywamy \textbf{nieokresowym}.
        \item Jeśli $o(j) = d > 1$, stan $j$ nazywamy \textbf{okresowym o okresie $d$}.
    \end{itemize}
\end{definition}

\begin{lemat}
    Wszystkie stany w tej samej klasie komunikującej się mają ten sam okres.
\end{lemat}

\begin{example}
    Dla $P = \begin{pmatrix} 0 & 1 \\ 1 & 0 \end{pmatrix}$, powrót do stanu 1 jest możliwy w 2, 4, 6, ... krokach.
    $o(1) = \NWD\{2, 4, 6, \dots\} = 2$. Podobnie $o(2)=2$.
\end{example}
...
\begin{example}
    ...
    Łańcuch jest nieprzywiedlny.
    Możliwe powroty do stanu 1: $1 \to 2 \to 1$ (2 kroki), $1 \to 3 \to 1$ (2 kroki), $1 \to 2 \to 1 \to 2 \to 1$ (4 kroki) itd. Zbiór długości to $\{2, 4, 6, \dots\}$. Zatem $o(1)=2$. Ponieważ wszystkie stany są w jednej klasie, $o(2)=o(3)=2$.
\end{example}

\begin{example}
    Rozważmy łańcuch z macierzą przejścia $P = \begin{pmatrix} 1/2 & 1/2 \\ 1 & 0 \end{pmatrix}$.
    ...
    Łańcuch ten jest nieprzywiedlny.
    Możliwe powroty do stanu 1: $1 \to 1$ (1 krok), $1 \to 2 \to 1$ (2 kroki), $1 \to 1 \to 1$ (2 kroki), $1 \to 2 \to 1 \to 1$ (3 kroki) itd.
    Zbiór długości powrotów do stanu 1 zawiera $\{1, 2, 3, \dots\}$. Zatem $o(1) = \NWD\{1, 2, 3, \dots\} = 1$. Stan 1 jest nieokresowy.
    Ponieważ łańcuch jest nieprzywiedlny i jeden ze stanów (stan 1) jest nieokresowy, cały łańcuch jest nieokresowy.
\end{example}

Ostatecznie, warunkiem koniecznym i wystarczającym na zbieżność do unikalnego rozkładu stacjonarnego jest to, aby łańcuch był nieprzywiedlny i nieokresowy.
